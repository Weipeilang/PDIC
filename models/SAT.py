...
class EncoderDecoder(nn.Module):
    ...
    other models.
    """
    def __init__(self, encoder, decoder, src_embed, tgt_embed, generator, generator_length):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask, token):
        encoder = self.encode(src, src_mask)
        length = self.generator_length(encoder)
        "Take in and process masked src and target sequences."
        return self.decode(encoder,src_mask,
                            tgt, tgt_mask, token), length
    
    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)
    
    def decode(self, memory, src_mask, tgt, tgt_mask, token):
        return self.decoder(self.tgt_embed([tgt, token]), memory, src_mask, tgt_mask)
...
class Generator_length(nn.Module):
    "Define standard linear + softmax generation step."
    def __init__(self, d_model, vocab):
        super(Generator_length, self).__init__()
        self.proj = nn.Linear(d_model, vocab)

    def forward(self, x):
        return self.proj(torch.sum(x, dim=1))
...
def relaxed_subsequent_mask(length, token):
    mask = []
    a = []
    for t in token:
        a += [i for i in t]
    for t in a:
        m = np.zeros([length, length]).astype('uint8')
        start, end = t[0], t[0]
        m[:, :end] = 1
        for l in t[1:]:
            m[start:, :end + int(l)] = 1
    ...
    def forward(self, x, token=None):
        ...
        if len(tokens) == 5 or len(tokens) == 1:
            for i in range(len(tokens)):
                new_x = torch.zeros(sum(tokens[i]), x.size(-1))
                start, end = 0, 4
                start_new = 0
                for j in tokens[i]:
                    t = torch.sum(x[i, start:end], dim=0) / (end - start)
                    t = t.repeat(j, 1)
                    new_x[start_new:start_new + j] = t
                    start_new += j
                    start = end
                    end = start + j

                x_.append(new_x)
            x = torch.stack(x_, dim=0).to(x.device)
        else:
            for i in range(len(tokens)):
                new_x = torch.zeros_like(x[i])
                start, end = 0, 4
                start_new = 0
                for j in tokens[i]:
                    t = torch.sum(x[i, start:end], dim=0)  / (end - start)
                    ...
                    if end > len(new_x):
                        break
                if start_new < len(new_x):
                    new_x[start_new:] = x[0, :1].repeat(len(new_x) - start_new, 1)
                x[i] = new_x
        return x
...
class SAT(AttModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        length = self.model.generator_length(memory)

        return fc_feats[...,:1], att_feats[...,:1], memory, att_masks, length

    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None, token=None):
        ...
        if seq is not None:
            ...
            seq_mask = seq_mask & relaxed_subsequent_mask(seq.size(-1), token).to(seq_mask)
        else:
            seq_mask = None

        return att_feats, seq, att_masks, seq_mask

    def _forward(self, fc_feats, att_feats, seq, att_masks=None, token=None):
        # pdb.set_trace()
        ...
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq, token)

        out, length = self.model(att_feats, seq, att_masks, seq_mask, token)
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask, token):
        """
        ...
        out = self.model.decode(memory, mask, 
                               ys, relaxed_subsequent_mask(sum(token[0]), [token]).to(memory.device), [token])

        #         out = self.model.decode(memory, mask,
        #                                ys,
        #                                relaxed_subsequent_mask(ys.size(1), self.opt.K)
        #                                         .to(memory.device))
        return out[:, -token[0][-1]:], [ys.unsqueeze(0)]


    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, token):
        # 'it' contains a word index
        # pdb.set_trace()
        xt = self.embed(it)

        output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, token)
        logprobs = F.log_softmax(self.logit(output), dim=-1)

        return logprobs, state


    def _sample(self, fc_feats, att_feats, att_masks=None, token=None, opt={}):
        ...
        # if att_feats.size(0) > 1:
        #     fc_feats, att_feats, att_masks = fc_feats[:1], att_feats[:1], att_masks[:1]
        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size)
        # pdb.set_trace()

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, length = self._prepare_feature(fc_feats, att_feats, att_masks)
        # pdb.set_trace()
        # pp_att_feats = pp_att_feats[:1]
        ...
        # _, token = torch.max(length.reshape(5, 4), dim=-1)
        # token = (token + 1).tolist()

        if token[0] == []:
            token = [4,4,4,4,4]
        else:
            token = [int(i) for i in np.around(np.sum(np.array(list(token))[0], axis=0)/5)]
        for t in range(len(token)):
            if t == 0: # input <bos>
                it = fc_feats.new_zeros((batch_size,self.opt.K), dtype=torch.long)
            # pdb.set_trace()
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, [token[:t+1]] * batch_size)
            ...
            #     break
            if sum(token[:t+1]) > self.seq_length: # skip if we achieve maximum length
                break
            if sample_max:
            ...
            if t == 0:
                seq[:, 0:token[t]] = it
                seqLogprobs[:, 0:token[t]] = sampleLogprobs
            else:
                seq[:, sum(token[:t]):sum(token[:t+1])] = it
                seqLogprobs[:, sum(token[:t]):sum(token[:t+1])] = sampleLogprobs
