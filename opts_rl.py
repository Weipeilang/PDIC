...
def parse_opt():
    ...
    # Data input settings
    parser.add_argument('--input_json', type=str, default='../satic-main/data/cocotalk.json',  help='path to the json file containing additional info and vocab')
    parser.add_argument('--input_fc_dir', type=str, default='../satic-main/data/mscoco/cocobu_fc',help='path to the directory containing the preprocessed fc feats')
    parser.add_argument('--input_att_dir', type=str, default='../satic-main/data/mscoco/cocobu_att', help='path to the directory containing the preprocessed att feats')
    parser.add_argument('--input_box_dir', type=str, default='../satic-main/data/mscoco/cocobu_box', help='path to the directory containing the boxes of att feats')
    ###################################################################################
    parser.add_argument('--input_label_h5', type=str, default='../satic-main/data/cocotalk_label.h5', help='path to the h5file containing the preprocessed dataset')
    ###################################################################################
    parser.add_argument('--start_from', type=str, default='save/nsc-sat-4-from-nsc-seqkd',
                    ...
                    """)
    parser.add_argument('--cached_tokens', type=str, default='../satic-main/data/coco-train-idxs', help='Cached token file for calculating cider score during self critical training.')

    # Model settings
    ############################################################################
    parser.add_argument('--caption_model', type=str, default="sat",  help='show_tell, show_attend_tell, all_img, fc, att2in, att2in2, att2all2, adaatt, adaattmo, topdown, stackatt, denseatt, transformer, sat')
    ############################################################################
    parser.add_argument('--rnn_size', type=int, default=2048, help='size of the rnn in number of hidden nodes in each layer')
    parser.add_argument('--num_layers', type=int, default=6, help='number of layers in the RNN')
    ...
    parser.add_argument('--self_critical_after', type=int, default=10, help='After what epoch do we start finetuning the CNN? (-1 = disable; never finetune, 0 = finetune from start)')
    ...
    parser.add_argument('--noamopt', default=True, help='')
    ...
    parser.add_argument('--save_checkpoint_every', type=int, default=6000, help='how often to save a model checkpoint (in iterations)?')
    parser.add_argument('--save_history_ckpt', type=int, default=0,help='If save checkpoints at every save point')
    #######################################################################################
    parser.add_argument('--checkpoint_path', type=str, default='save/nsc-sat-4-from-nsc-seqkd', help='directory to store checkpointed models')
    #######################################################################################
    ...
    # misc
    #######################################################################################
    parser.add_argument('--id', type=str, default='sat-4-from-nsc-seqkd', help='an id identifying this run/job. used in cross-val and appended when writing progress files')
    #######################################################################################
    ...
    # New add
    #########################################################################################
    parser.add_argument('--kd_weight', type=float, default=0, help='The knowledge distillation weight.')
    parser.add_argument('--teacher_caption_model', type=str, default="show_tell", help='show_tell, show_attend_tell, all_img, fc, att2in, att2in2, att2all2, adaatt, adaattmo, topdown, stackatt, denseatt, transformer')
    #########################################################################################
    parser.add_argument('--K', type=int, default=4, help='Number of parallel of SAT model.')
    parser.add_argument('--nsc', type=bool, default= True,help='Whether using the new self critical.')
