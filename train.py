...
def train(opt):
    ...
    ##############################################################
    # Force opt.use_att = True temporary
    opt.use_att = True
    opt.use_fc = True
    ##############################################################
    ...
    #================================================
    # pdb.set_trace()
    #Load a pre-trained model as teacher.
    ...
    # Remember to change the pre-trained model path according to opt.teacher_caption_model.
    ###########################################################################
    if opt.teacher_caption_model == 'topdown':
        teacher.load_state_dict(torch.load('save/up-down-baseline/model-best.pth'))
    elif opt.teacher_caption_model == 'show_tell':
        teacher.load_state_dict(torch.load('../satic-main/save/student-showtell-teacher-updown-1.5/model-best.pth'))
    ...
    if (opt.start_from == None) and (opt.caption_model == 'sat') and (opt.K > 1):
        transformer_state_dict = torch.load('../satic-main/save/nsc-transformer-baseline/model-best.pth')
        model_dict = model.state_dict()
        petrained_dict = {k : v for k, v in transformer_state_dict.items() if k in model_dict}
        model_dict.update(petrained_dict)
        model.load_state_dict(model_dict)
    ####################################################

    for name, p in model.named_parameters():
        if name.startswith('model.generator_length'):
    ...
    while True:
        ...
        # if data['token'] == ([],):
        #     data['token'] = ([np.array([2, 1, 2, 1, 2, 3, 3, 3]), np.array([3, 1, 2, 1, 2, 3, 3, 3]), np.array([2, 1, 2, 1, 2, 1, 2, 3]), np.array([2, 1, 2, 1, 2, 1, 2, 3]), np.array([2, 1, 3, 1, 2, 3, 3, 3])] )
        ...
        optimizer.zero_grad()
        model_out = dp_lw_model(fc_feats, att_feats, labels, masks, att_masks, data['gts'], data['token'], torch.arange(0, len(data['gts'])), sc_flag)
        ...
        if (iteration % opt.losses_log_every == 0):
            ...
            if sc_flag:
                add_summary_value(tb_summary_writer, 'avg_reward', 0, iteration)
